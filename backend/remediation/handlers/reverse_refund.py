"""Handler for REVERSE_REFUND execution adjusting ledger credits and reversing invalid double-debit."""
from datetime import datetime, timezone
import uuid
from typing import Any, Dict, Tuple
from sqlalchemy.orm import Session
from sqlalchemy import select

from backend.models.enums import DisputeEventType, LedgerEntryType, PaymentStatus
from backend.models.financial_sources import GatewayTransaction, DisputeRefundEvent, NodalLedgerEntry
from backend.models.remediation import RemediationAction
from backend.remediation.handlers.base import BaseActionHandler


class ReverseRefundHandler(BaseActionHandler):
    """Executes refund reversal, creating offsetting reversal event and ledger credit."""

    def execute(
        self,
        session: Session,
        plan: RemediationAction,
        parameters: Dict[str, Any],
    ) -> Tuple[Dict[str, Any], Dict[str, Any], str]:
        payment_id = parameters["payment_id"]
        amount = parameters["amount_minor_units"]
        reason = parameters.get("reason", "Reverse duplicate refund / double dip liability")

        gt = session.scalars(select(GatewayTransaction).where(GatewayTransaction.payment_id == payment_id)).first()
        last_ledger = session.scalars(select(NodalLedgerEntry).order_by(NodalLedgerEntry.id.desc())).first()
        prev_balance = last_ledger.balance_after if last_ledger else 0

        before_snapshot = {
            "payment_id": payment_id,
            "payment_status": gt.status if gt else "UNKNOWN",
            "ledger_balance_before": prev_balance,
        }

        now = datetime.now(timezone.utc)

        # 1. Record Reversal Event
        reversal_event_id = f"REV-{uuid.uuid4().hex[:12]}"
        rev_evt = DisputeRefundEvent(
            event_id=reversal_event_id,
            payment_id=payment_id,
            event_type=DisputeEventType.CHARGEBACK_REVERSAL.value,
            amount=amount,
            timestamp=now,
            reason_code=reason,
        )
        session.add(rev_evt)

        # 2. Post offsetting ledger credit entry
        new_balance = prev_balance + amount
        ledger_entry_id = f"NLE-REV-{uuid.uuid4().hex[:8]}"
        entry = NodalLedgerEntry(
            ledger_id=ledger_entry_id,
            transaction_id=payment_id,
            account_id="nodal_escrow_main",
            entry_type=LedgerEntryType.SETTLEMENT_CREDIT.value,
            debit=0,
            credit=amount,
            balance_after=new_balance,
            reference=f"Remediation refund reversal for {plan.exception_id}: {reason}",
            timestamp=now,
        )
        session.add(entry)

        # 3. Restore gateway status if needed
        if gt and gt.status == PaymentStatus.REFUNDED.value:
            gt.status = PaymentStatus.CAPTURED.value

        session.flush()

        after_snapshot = {
            "payment_id": payment_id,
            "reversal_event_id": reversal_event_id,
            "ledger_entry_id": ledger_entry_id,
            "ledger_balance_after": new_balance,
            "reversal_amount": amount,
        }

        summary = f"Processed ₹{amount/100:,.2f} refund reversal on {payment_id} with ledger credit {ledger_entry_id}."
        return before_snapshot, after_snapshot, summary

    def dry_run(
        self,
        session: Session,
        plan: RemediationAction,
        parameters: Dict[str, Any],
    ) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        payment_id = parameters.get("payment_id")
        amount = parameters.get("amount_minor_units", 0)

        last_ledger = session.scalars(select(NodalLedgerEntry).order_by(NodalLedgerEntry.id.desc())).first()
        prev_balance = last_ledger.balance_after if last_ledger else 0

        before_snapshot = {"payment_id": payment_id, "ledger_balance": prev_balance}
        after_snapshot = {
            "payment_id": payment_id,
            "projected_ledger_balance": prev_balance + amount,
            "projected_credit": amount,
        }
        return before_snapshot, after_snapshot
